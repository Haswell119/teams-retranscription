...
| `DRIFT_PROBE_COUNT` | int | `8` | Number of probe windows, spread evenly across detected speech. Eight 4-second probes are 32 seconds of audio: about 9 % of a 6-minute recording and under 1 % of an hour-long meeting. |
| `DRIFT_LADDER_SECONDS` | list[float] | `[15.0, 8.0, 4.0]` | Segment ceilings tried in turn once drift is detected, longest first. The guard stops at the first rung where the probed language accounts for at least 75 % of the transcript, and keeps the best attempt if none reaches it. Rungs at or above `AUDIO__MAX_SEGMENT_SECONDS` are skipped, so the guard never lengthens segments. |
...
| `CLEAN_EMBEDDING_SAMPLES` | bool | `true` | Which segments consolidation extracts speaker embeddings from. With it on, a cluster's samples are its **longest turns among those the segmentation model does not also assign to somebody else**; with it off, simply its longest turns, which on a densely overlapped meeting are exactly the most contaminated ones. |
| `MAXIMUM_SAMPLE_CONTEST` | float | `0.2` | How much of a turn another speaker may be talking through before it stops counting as a clean embedding sample. Turns above it are still usable as a last resort, ranked behind the clean ones, so a cluster is never left without a centroid. Raise it toward `1.0` to weight duration more and cleanliness less. |
...
HANSARD_DIARIZATION__MAXIMUM_SAMPLE_CONTEST=0.2
